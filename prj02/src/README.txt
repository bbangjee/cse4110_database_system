...
#include <mysql.h>   // MySQL 연결, 기존 사이버 캠퍼스 파일의 헤더형태와 다름
...
1. SSL 연결 문제 해결
   ...
   MySQL Connector C 6.1에서 SSL 연결 오류 발생 때문에 다음 코드를 추가:
...
2. 인증 플러그인 문제 해결
   ...
   MySQL 8.0의 기본 인증 방식(caching_sha2_password)이 
   MySQL Connector C 6.1에서 지원되지 않음
   
   문제 메시지:
   Connection failure: Authentication plugin 'caching_sha2_password' 
   cannot be loaded
...
3. 런타임 라이브러리 의존성
   ...
   실행 시 libmysql.dll이 꼭!! 필요하므로 소스 파일과 같은 폴더에 배치했습니다.
